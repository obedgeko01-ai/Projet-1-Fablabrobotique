...
class AdminUtilisateursModele
{
    ...
    {
        $sql = "SELECT id, nom, email, role, cree_le FROM utilisateurs ORDER BY cree_le DESC";
    ...
    public function create(array $data): int
    {
        ...
        $sql  = "INSERT INTO utilisateurs (nom, email, mot_de_passe, role, cree_le)
    ...
    public function update(int $id, array $data): bool
    {
        if (!empty($data['mot_de_passe'])) {
            $sql  = "UPDATE utilisateurs SET nom=:nom, email=:email, role=:role, mot_de_passe=:mot_de_passe WHERE id=:id";
            $stmt = $this->db->prepare($sql);
            return $stmt->execute([
                ':nom'          => $data['nom'],
                ':email'        => $data['email'],
